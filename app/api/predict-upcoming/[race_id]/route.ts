...
import { calcDangerScore } from '@/lib/dangerScore'
...
interface Bloodline { sire_line: string | null; bms_line: string | null }

async function fetchHorseBloodlines(horses: HorseWithHistory[]): Promise<Map<string, Bloodline>> {
  const names = horses.map(h => h.horse_name)
  const { data } = await supabase.from('horses').select('name, sire_line, bms_line').in('name', names)
  const map = new Map<string, Bloodline>()
  for (const h of data ?? []) {
    map.set(h.name as string, { sire_line: h.sire_line as string | null, bms_line: h.bms_line as string | null })
  }
  return map
}
...
export async function POST(
  ...
  const buildHorseLine = (h: HorseWithHistory, rank?: number) => {
    ...
    const bl = bloodlineMap.get(h.horse_name)
    const bloodlineNote = bl?.sire_line
      ? `父系:${bl.sire_line}${bl.bms_line ? `・母父系:${bl.bms_line}` : ''}`
      : ''
    return `${prefix}${h.horse_name}（${jInfo} / ${indexInfo}${recent ? ' / 直近:' + recent : ''}${hw ? ' / ' + hw : ''}${extras ? ' / ' + extras : ''} / ${conditionNote}${bloodlineNote ? ' / ' + bloodlineNote : ''}）`
  }
...
推定馬場: ${weather.trackEstimate}（${weather.trackNote}）`
  ...
  // 危険人気馬判定（指数上位3頭のリスクチェック）
  const topByIndex = [...horses]
    .filter(h => h.avg_time_index != null)
    .sort((a, b) => (b.avg_time_index ?? 0) - (a.avg_time_index ?? 0))
    .slice(0, 3)
  const dangerLines = topByIndex
    .map(h => {
      const d = calcDangerScore(h, race.distance, race.track_type, horses)
      if (d.score < 3) return null
      return `・${h.horse_name}: ${d.flags.join('・')}（危険スコア: ${d.score}）`
    })
    .filter((l): l is string => l !== null)
  const dangerSection = dangerLines.length > 0
    ? `【危険人気馬チェック】\n${dangerLines.join('\n')}\n上記は上位人気が見込まれるが複数のリスク要因を持つ。分析時に注意すること。`
    : ''
...
[BETTING_PATTERNS]
honmei|本命パターン（単勝・馬連）の根拠を30字以内で
balance|バランスパターン（三連複）の軸馬選択根拠を30字以内で
ana|穴狙いパターンの注目馬とその理由を30字以内で
[/BETTING_PATTERNS]
`.trim()
